...
colored by high acceleration events

Usage:
    TSNE_NUM_STEPS=1000 TSNE_ACCELERATION_THRESHOLD=10.0 python run_tsne_high_acceleration.py --task=go2_blind --checkpoint=13000
"""
...
# Add parent directory to path (insert at beginning to prioritize local imports)
...
def main():
    ...
    save_path = os.environ.get('TSNE_SAVE_PATH', 'tsne_high_acceleration.png')
    perplexity = int(os.environ.get('TSNE_PERPLEXITY', '30'))
    acceleration_threshold = float(os.environ.get('TSNE_ACCELERATION_THRESHOLD', '10.0'))

    print(f"\nVisualization settings:")
    print(f"  - num_steps: {num_steps}")
    print(f"  - use_full_state: {use_full_state}")
    print(f"  - color_by: high_acceleration (joint acceleration > threshold)")
    print(f"  - acceleration_threshold: {acceleration_threshold} rad/s²")
    print(f"  - save_path: {save_path}")
    print(f"  - perplexity: {perplexity}")
    print(f"\nTip: Set environment variables to customize (e.g., TSNE_ACCELERATION_THRESHOLD=15.0)")
    ...
    print(f"Coloring by high acceleration events (threshold: {acceleration_threshold} rad/s²)")

    # Import visualization module here (after isaacgym/torch imports are done)
    from visualize_high_acceleration import visualize_high_acceleration_from_checkpoint

    # Run visualization
    visualizer, tsne_results = visualize_high_acceleration_from_checkpoint(
    ...
    # Print high acceleration statistics
    if len(visualizer.metadata['high_acceleration']) > 0:
        high_accel = np.concatenate(visualizer.metadata['high_acceleration'])
        num_high = np.sum(high_accel)
        print(f"High acceleration events (>{acceleration_threshold} rad/s²): {num_high} / {len(high_accel)} ({100*num_high/len(high_accel):.2f}%)")
