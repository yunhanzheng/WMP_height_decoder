"""
Standalone script to generate BOTH reward and feet_stumble visualizations
from a SINGLE rollout to ensure they're based on identical data

Usage:
    TSNE_NUM_STEPS=1000 TSNE_PERPLEXITY=20 python run_tsne_comparison.py --task=go2_blind --checkpoint=13000
"""
...
def main():
    ...
    print(f"  - output: tsne_comparison.png (combined), tsne_reward.png, tsne_stumble.png")
    ...
    print(f"Will generate both reward and feet_stumble visualizations from the same data")

    # Import visualization module here (after isaacgym/torch imports are done)
    from visualize_comparison import visualize_comparison_from_checkpoint

    # Run visualization
    visualizer, tsne_results = visualize_comparison_from_checkpoint(
    ...
    # Print feet stumble statistics
    if len(visualizer.metadata['feet_stumble']) > 0:
        feet_stumble = np.concatenate(visualizer.metadata['feet_stumble'])
        num_stumbles = np.sum(feet_stumble)
        print(f"Feet stumble events: {num_stumbles} / {len(feet_stumble)} ({100*num_stumbles/len(feet_stumble):.2f}%)")

    # Print reward statistics
    if len(visualizer.metadata['rewards']) > 0:
        rewards = np.concatenate(visualizer.metadata['rewards'])
        print(f"Reward statistics: mean={np.mean(rewards):.3f}, std={np.std(rewards):.3f}, min={np.min(rewards):.3f}, max={np.max(rewards):.3f}")
