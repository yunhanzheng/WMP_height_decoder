...
class G1Robot(LeggedRobot):
    ...
    def _reward_feet_swing_height(self):
        # Target = terrain height under foot + 0.08 m clearance (swing feet only)
        contact = torch.norm(self.contact_forces[:, self.feet_indices, :3], dim=2) > 1.0
        target_z = self._get_feet_terrain_heights() + 0.08
        pos_error = torch.square(self.feet_pos[:, :, 2] - target_z) * ~contact
