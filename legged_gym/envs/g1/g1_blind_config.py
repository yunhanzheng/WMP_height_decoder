...
class G1BlindCfg(G1BaseCfg):
    class asset(G1BaseCfg.asset):
        visualize_ghost = False  # ghost-robot viz relies on quadruped foot IK; keep off
    ...
    class env(LeggedRobotCfg.env):
        ...
        # Privileged prefix for the G1 biped (2 feet, 8 penalized contact bodies):
        #   p_gains(12) + d_gains(12)                                = 24
        #   contact_flag(8) + contact_force(2*3=6) + com_pos(3)
        #     + base_mass(1) + restitution(1) + friction(1)          = 20
        #   base_lin_vel(3)                                          =  3
        privileged_dim = 24 + 20 + 3  # = 47
        height_dim = 187   # 17 x 11 measured points
        footprint_dim = 33 * 21  # base-centered footprint grid for the binary height decoder
